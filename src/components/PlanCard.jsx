...
export function PlanCard({ plan, onSelect }) {
  ...
  return (
    <motion.article
      layout
      whileHover={isOutOfStock ? undefined : { y: -6, scale: 1.01 }}
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {isOutOfStock ? (
          <span className="inline-flex items-center gap-2 rounded-full border border-rose-200/20 bg-rose-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-rose-100">
            Out of stock
          </span>
        ) : plan.highlight ? (
      ...
      <div className="mt-6 flex items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs uppercase tracking-[0.18em] text-zinc-500">{plan.stockLabel || "Available"}</p>
        </div>
        <Button onClick={() => onSelect(plan)} disabled={isOutOfStock} className="gap-2">
          {isOutOfStock ? "Unavailable" : "Select Plan"}
